...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f8f9fa;">
    <div style="background-color: white; border-radius: 12px; padding: 30px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
        <div style="text-align: center; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 2px solid #e9ecef;">
            <h1 style="color: #2c3e50; margin: 0 0 10px 0; font-size: 28px;">🍽️ Your Weekly Meal Plan</h1>
            ...
            ${days.map((day, index) => {
              ...
              return `
                <div style="margin-bottom: 25px; border: 1px solid #e9ecef; border-radius: 8px; overflow: hidden;">
                    <div style="background-color: #495057; color: white; padding: 12px 20px; font-weight: 600; font-size: 16px;">${dayNames[index]}</div>
                    <div style="padding: 20px;">
                        ${enabledMeals.map(mealType => {
                          const mealName = dayMeals[mealType];
                          return `
                            <div style="margin-bottom: 15px; padding: 12px; background-color: #f8f9fa; border-radius: 6px; border-left: 4px solid #007bff;">
                                <div style="font-weight: 600; color: #495057; margin-bottom: 5px; font-size: 14px; text-transform: uppercase; letter-spacing: 0.5px;">${mealTypeLabels[mealType] || mealType}</div>
        ...
        <div style="text-align: center; padding-top: 20px; border-top: 1px solid #e9ecef; color: #6c757d; font-size: 14px;">
            ...
            <p style="margin: 0 0 15px 0;">
                <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://your-app.vercel.app'}" style="color: #007bff; text-decoration: none;">Visit your meal planner</a> 
